<template>
  <div class="h-screen w-screen bg-transparent text-slate-800 flex flex-col overflow-hidden font-sans selection:bg-indigo-500 selection:text-white">
    <header class="h-16 border-b border-slate-200 bg-white/90 backdrop-blur px-6 flex items-center justify-between z-30 shrink-0">
      <div class="flex items-center space-x-4">
        <button @click="backToHome" class="p-2 text-slate-500 hover:text-slate-800 bg-slate-100/80 hover:bg-slate-200 rounded-xl transition cursor-pointer" title="Back to Dashboard">
          <ArrowLeftIcon class="w-5 h-5" />
        </button>
        <div class="flex items-center space-x-2">
          <div @click="backToHome" class="cursor-pointer w-8 h-8 rounded-lg bg-gradient-to-tr from-indigo-600 to-cyan-400 flex items-center justify-center shadow">
      ...
      <div class="flex items-center space-x-3">
        ...
        <button @click="openTruthTableModal" class="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 text-indigo-600 rounded-xl text-xs font-bold border border-slate-200 transition flex items-center space-x-2">
          <TableIcon class="w-4 h-4" />
          <span>Truth Table</span>
        </button>
        <button @click="exportCircuitPDF" class="px-3.5 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold shadow-lg shadow-indigo-600/30 transition flex items-center space-x-2">
          <DownloadIcon class="w-4 h-4" />
          <span>Export PDF</span>
        </button>

        <button @click="confirmDeleteRoom" class="p-2 bg-rose-50 hover:bg-rose-500 text-rose-500 hover:text-white rounded-xl text-xs font-bold border border-rose-100 transition" title="Delete Room">
    ...
    <div class="flex-1 flex overflow-hidden relative">
      <aside class="w-64 border-r border-slate-200 bg-white/80 backdrop-blur p-4 flex flex-col space-y-6 shrink-0 overflow-y-auto">
        <div>
          <h3 class="text-xs font-bold uppercase tracking-wider text-slate-500 mb-3">Input Elements</h3>
          <div class="grid grid-cols-1 gap-2">
            <button draggable="true" @dragstart="onDragStartPanel($event, 'input-switch', 'Switch')" @click="addNode('input-switch', 'Switch')" class="p-3 bg-white hover:bg-indigo-50/80 border border-slate-200 hover:border-indigo-300 rounded-xl flex items-center justify-between text-xs font-semibold text-slate-700 transition group shadow-sm">
              <div class="flex items-center space-x-2">
        ...
        <div>
          ...
          <div class="grid grid-cols-2 gap-2">
            <!-- NOT Gate -->
            <button draggable="true" @dragstart="onDragStartPanel($event, 'gate-not', 'NOT Gate')" @click="addNode('gate-not', 'NOT Gate')" 
            ...
            <!-- AND Gate -->
            <button draggable="true" @dragstart="onDragStartPanel($event, 'gate-and', 'AND Gate')" @click="addNode('gate-and', 'AND Gate')" 
                    class="p-2.5 bg-white hover:bg-slate-50 border border-slate-200 hover:border-indigo-300 rounded-xl flex flex-col items-center justify-center transition group shadow-sm">
              <svg viewBox="0 0 60 30" class="w-10 h-5 stroke-slate-800 fill-white stroke-[2]">
                <line x1="2" y1="8" x2="15" y2="8" /><circle cx="2" cy="8" r="2" class="fill-white stroke-slate-800" />
                <line x1="2" y1="22" x2="15" y2="22" /><circle cx="2" cy="22" r="2" class="fill-white stroke-slate-800" />
                <path d="M 15,4 L 28,4 A 11,11 0 0,1 28,26 L 15,26 Z" />
                <line x1="39" y1="15" x2="58" y2="15" /><circle cx="58" cy="15" r="2" class="fill-white stroke-slate-800" />
              </svg>
              <span class="text-[10px] font-semibold text-slate-600 mt-1">AND Gate</span>
            </button>

            <!-- NAND Gate -->
            <button draggable="true" @dragstart="onDragStartPanel($event, 'gate-nand', 'NAND Gate')" @click="addNode('gate-nand', 'NAND Gate')" 
                    class="p-2.5 bg-white hover:bg-slate-50 border border-slate-200 hover:border-indigo-300 rounded-xl flex flex-col items-center justify-center transition group shadow-sm">
              <svg viewBox="0 0 60 30" class="w-10 h-5 stroke-slate-800 fill-white stroke-[2]">
                <line x1="2" y1="8" x2="15" y2="8" /><circle cx="2" cy="8" r="2" class="fill-white stroke-slate-800" />
                <line x1="2" y1="22" x2="15" y2="22" /><circle cx="2" cy="22" r="2" class="fill-white stroke-slate-800" />
                ...
                <line x1="43" y1="15" x2="58" y2="15" /><circle cx="58" cy="15" r="2" class="fill-white stroke-slate-800" />
              </svg>
              <span class="text-[10px] font-semibold text-slate-600 mt-1">NAND Gate</span>
            </button>

            <!-- OR Gate -->
            <button draggable="true" @dragstart="onDragStartPanel($event, 'gate-or', 'OR Gate')" @click="addNode('gate-or', 'OR Gate')" 
                    class="p-2.5 bg-white hover:bg-slate-50 border border-slate-200 hover:border-indigo-300 rounded-xl flex flex-col items-center justify-center transition group shadow-sm">
              <svg viewBox="0 0 60 30" class="w-10 h-5 stroke-slate-800 fill-white stroke-[2]">
                <line x1="2" y1="8" x2="15" y2="8" /><circle cx="2" cy="8" r="2" class="fill-white stroke-slate-800" />
                <line x1="2" y1="22" x2="15" y2="22" /><circle cx="2" cy="22" r="2" class="fill-white stroke-slate-800" />
                <path d="M 15,4 Q 28,4 38,15 Q 28,26 15,26 Q 22,15 15,4 Z" />
                <line x1="38" y1="15" x2="58" y2="15" /><circle cx="58" cy="15" r="2" class="fill-white stroke-slate-800" />
              </svg>
              <span class="text-[10px] font-semibold text-slate-600 mt-1">OR Gate</span>
            </button>

            <!-- NOR Gate -->
            <button draggable="true" @dragstart="onDragStartPanel($event, 'gate-nor', 'NOR Gate')" @click="addNode('gate-nor', 'NOR Gate')" 
                    class="p-2.5 bg-white hover:bg-slate-50 border border-slate-200 hover:border-indigo-300 rounded-xl flex flex-col items-center justify-center transition group shadow-sm">
              <svg viewBox="0 0 60 30" class="w-10 h-5 stroke-slate-800 fill-white stroke-[2]">
                <line x1="2" y1="8" x2="15" y2="8" /><circle cx="2" cy="8" r="2" class="fill-white stroke-slate-800" />
                <line x1="2" y1="22" x2="15" y2="22" /><circle cx="2" cy="22" r="2" class="fill-white stroke-slate-800" />
                ...
                <line x1="40" y1="15" x2="58" y2="15" /><circle cx="58" cy="15" r="2" class="fill-white stroke-slate-800" />
              </svg>
              <span class="text-[10px] font-semibold text-slate-600 mt-1">NOR Gate</span>
            </button>

            <!-- XOR Gate -->
            <button draggable="true" @dragstart="onDragStartPanel($event, 'gate-xor', 'XOR Gate')" @click="addNode('gate-xor', 'XOR Gate')" 
                    class="p-2.5 bg-white hover:bg-slate-50 border border-slate-200 hover:border-indigo-300 rounded-xl flex flex-col items-center justify-center transition group shadow-sm">
              <svg viewBox="0 0 60 30" class="w-10 h-5 stroke-slate-800 fill-white stroke-[2]">
                <line x1="2" y1="8" x2="15" y2="8" /><circle cx="2" cy="8" r="2" class="fill-white stroke-slate-800" />
                <line x1="2" y1="22" x2="15" y2="22" /><circle cx="2" cy="22" r="2" class="fill-white stroke-slate-800" />
        ...
        <div>
          ...
          <div class="grid grid-cols-1 gap-2">
            <button draggable="true" @dragstart="onDragStartPanel($event, 'output-bulb', 'Bulb')" @click="addNode('output-bulb', 'Bulb')" class="p-3 bg-white hover:bg-yellow-50/80 border border-slate-200 hover:border-yellow-300 rounded-xl flex items-center justify-between text-xs font-semibold text-slate-700 transition group shadow-sm">
              <div class="flex items-center space-x-2">
        ...
        <div class="pt-4 border-t border-slate-200 space-y-3">
          ...
          <div class="flex items-center justify-between text-xs text-slate-600">
            <span>Grid Size</span>
            <select v-model="gridSize" class="px-2 py-1 bg-white border border-slate-300 rounded-lg text-xs font-mono text-slate-700">
          ...
          <div class="flex items-center justify-between text-xs text-slate-600">
            ...
            <input v-model="snapToGrid" type="checkbox" class="accent-indigo-500 w-4 h-4 rounded" />
          </div>

          <div class="pt-2 text-[11px] text-slate-500 leading-tight">
            💡 Select node or wire and press <kbd class="px-1 bg-slate-100 border border-slate-200 rounded font-mono text-slate-600">Delete</kbd> or hover gate to click <span class="text-rose-500 font-bold">✕</span> to delete.
          </div>
        </div>
      </aside>
      ...
      <main id="circuit-canvas-area" class="flex-1 relative bg-transparent">
        <VueFlow v-model:nodes="nodes" 
          ...
          <Background :gap="gridSize" pattern-color="#cbd5e1" />
        ...
        <div class="absolute inset-0 pointer-events-none z-50 overflow-hidden" data-html2canvas-ignore="true">
          <div v-for="(cursor, userName) in remoteCursors" :key="userName"
            ...
            <svg class="w-5 h-5 drop-shadow-md" viewBox="0 0 24 24" :fill="cursor.color || '#4361ee'" stroke="white" stroke-width="2" stroke-linejoin="round">
    ...
    <TruthTableModal :is-open="showTruthTable" :loading="truthTableLoading" :table-data="truthTableData" @close="showTruthTable = false" />
  </div>
</template>
  ...
  mercureEventSource.onmessage = (event) => {
    try {
      ...
      if (data.graph && data.version > currentVersion.value) {
        ...
        if (data.gridSize) gridSize.value = data.gridSize;
        runLogicEngine();
      }
    } catch (err) {
